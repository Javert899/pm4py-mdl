...
def repr_tree(tree, viz, current_node, rec_depth, acti_map, persp_color, parameters):
    """
    ...
    for child in tree.children:
        if child.operator is None:
            ...
            if child.label is None:
                viz.node(this_trans_id, "tau", style='filled', fillcolor=persp_color)
            else:
            ...
            if condition_wo_operator:
                ...
                viz = repr_tree(child, viz, op_node_identifier, rec_depth + 1, acti_map, persp_color, parameters)
    return viz
...
def apply(res, parameters=None):
    ...
    for tree in res["models"].values():
        ...
        if tree.operator:
            ...
            viz = repr_tree(tree, viz, op_node_identifier, 0, acti_map, persp_color, parameters)
        else:
            viz.attr('node', shape='box', fixedsize='true', width="2.5",
                     fontsize="8")
            this_trans_id = str(uuid.uuid4())
